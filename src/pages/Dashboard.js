...
const Dashboard = () => {
  ...
  const location = useLocation(); // Для підсвічування активної кнопки
...
//   // ВИПРАВЛЕНО: перевіряємо '/' та '/dashboard' для головної
//   const isActive = (path) => {
...
//   return (
//     // ВИПРАВЛЕНО: h-screen + overflow-hidden на кореневому div щоб aside і main займали весь екран
...
//       {/* 🧭 Sidebar */}
//       {/* ВИПРАВЛЕНО: h-full замість h-screen щоб sidebar точно відповідав батьківському flex-контейнеру */}
//       <aside className={`
//         ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}
//         md:translate-x-0
//         fixed md:relative h-full w-72 border-r border-slate-800/60 bg-[#0B1120] z-50 flex flex-col transition-transform duration-300
//       `}>
//         <button
//           onClick={() => setIsMobileMenuOpen(false)}
...
//           <nav className="space-y-2">
//             {/* ВИПРАВЛЕНО: navigate('/dashboard') і виправлено isActive */}
//             <button
...
//         {/* ВИПРАВЛЕНО: mt-auto коректно працює бо aside — flex flex-col */}
//         <div className="mt-auto p-8 space-y-2 border-t border-slate-800/50">
//           <button
//             onClick={() => navigate('/profile')}
//             className="w-full flex items-center gap-3 px-4 py-3 text-slate-400 hover:text-white hover:bg-slate-800/50 rounded-xl transition-all"
//           >
...
//       {/* 📄 Головний контент */}
//       {/* ВИПРАВЛЕНО: h-full + overflow-y-auto щоб скрол був всередині, а не зовні */}
//       <main className="flex-1 h-full overflow-y-auto">
//         {/* 📱 Мобільний хедер */}
//         <div className="md:hidden flex items-center justify-between p-4 bg-[#0B1120] border-b border-slate-800 sticky top-0 z-30">
//           <span className="text-xl font-bold text-white">Hack<span className="text-indigo-400">Face</span></span>
//           <button onClick={() => setIsMobileMenuOpen(true)} className="p-2 bg-slate-800 rounded-lg">
//             <Menu size={24} />
//           </button>
//         </div>
...
//                 {/* ДОДАНО: клікабельний аватар — веде на /profile */}
//                 <button
//                   onClick={() => navigate('/profile')}
//                   className="flex items-center gap-4 bg-slate-900/50 backdrop-blur-md p-3 pr-6 rounded-2xl border border-slate-700/50 hover:border-indigo-500/60 hover:bg-slate-800/60 transition-all group self-start md:self-auto"
//                 >
//                   <div className="w-12 h-12 bg-gradient-to-tr from-indigo-600 to-purple-600 rounded-xl flex items-center justify-center text-white text-xl font-bold shadow-lg group-hover:scale-105 transition-transform">
//                     {user.name.charAt(0).toUpperCase()}
//                   </div>
//                   <div className="text-left">
//                     <p className="text-sm font-bold text-white leading-tight">{user.name}</p>
...
//                           <div
//                             key={t._id}
//                             className="p-6 bg-slate-800/40 rounded-2xl border border-slate-700/50 hover:border-indigo-500/50 transition-all group cursor-pointer"
//                           >
...
//                           <button
//                             onClick={() => navigate('/hackathons')}
//                             className="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl transition-all flex items-center gap-2 shadow-lg shadow-indigo-600/30"
//                           >
//                             <Search size={18} /> Знайти команду
//                           </button>
//                           {/* ДОДАНО: другорядна дія */}
//                           <button
//                             onClick={() => navigate('/teams/create')}
//                             className="px-6 py-3 bg-slate-800/60 hover:bg-slate-700/60 text-slate-300 hover:text-white font-medium rounded-xl transition-all border border-slate-700/50"
//                           >
//                             Створити команду
//                           </button>
//                         </div>
...
//                   {/* Проєкти — ВИПРАВЛЕНО: назва "Останні проєкти" щоб не дублювати меню */}
//                   <div className="bg-slate-900/40 border border-slate-800/60 rounded-[2rem] p-8 backdrop-blur-sm">
//                     <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
//                       <Code className="text-indigo-400" size={24} /> Останні проєкти
//                     </h3>
...
//                           <div
//                             key={p._id}
//                             className="p-5 bg-slate-800/30 rounded-2xl border border-slate-700/30 hover:bg-slate-800/50 transition-all"
//                           >
//                             <h4 className="font-bold text-white mb-1">{p.name}</h4>
//                             <p className="text-xs text-slate-500 line-clamp-1 mb-3">{p.description}</p>
//                             <a
...
//                     ) : (
//                       <div className="p-8 border border-slate-800 border-dashed rounded-2xl text-center">
//                         <p className="text-sm text-slate-500 font-medium">Проєктів поки немає</p>
//                       </div>
//                     )}
//                   </div>
//                 </div>
